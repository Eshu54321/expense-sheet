...
export const RecurringExpenses: React.FC<RecurringExpensesProps> = ({
    ...
    const expensesByDay = React.useMemo(() => {
        ...
        recurringExpenses.filter(e => e.active).forEach(item => {
            const day = new Date(item.nextDueDate).getDate();
            const current = map.get(day) || 0;
            map.set(day, current + item.amount);
        });
    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {/* Header Card */}
                <div className="md:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex justify-between items-center">
                    <div>
                ...
                <div className="bg-indigo-600 p-6 rounded-xl shadow-sm text-white flex flex-col justify-center">
                    <p className="text-indigo-100 text-xs font-medium uppercase tracking-wider mb-1">Total Monthly Commitment</p>
                    <h3 className="text-2xl font-bold">₹{totalMonthly.toFixed(0)}</h3>
                    <p className="text-xs text-indigo-200 mt-1">Estimated monthly outflow</p>
                </div>
            </div>
            ...
            {expensesByDay.length > 0 && (
                <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 overflow-x-auto">
                    <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Projected Outflow by Day</h3>
                    <div className="flex gap-4 min-w-max">
                        {expensesByDay.map(({ day, amount }) => (
                            <div key={day} className="flex flex-col items-center bg-slate-50 p-3 rounded-lg min-w-[80px] border border-slate-100">
                                <span className="text-xs font-semibold text-slate-400 mb-1">{day}{day === 1 ? 'st' : day === 2 ? 'nd' : day === 3 ? 'rd' : 'th'}</span>
